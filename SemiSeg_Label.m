...
function [CellList, allLabelled] = SemiSeg_Label(singleFrame, inputCellList)
    %Start up
    sz = size(singleFrame);
    Nx = sz(2);
    Ny = sz(1);
    ...
    Min = 0; Max = 5000;
    mainfig = figure;
    H = imshow(singleFrame, [Min Max]); title('Select rois to Label');
    ...
    rois_type = questdlg('Select ROI Type','What Code Generated the ROIs?','Simons','Kyles','Kyles');
    switch rois_type
        case 'Simons'
    ...
    while(true)
        ...
        selx = round(clickx);
        sely = round(clicky);
        selpixels = sub2ind(sz, sely, selx);
        for step = 1:numel(inputCellList)
            temp = intersect(inputCellList(step).(pixel_list), selpixels);    
            if ~isempty(temp)
                selrois(pos) = step;
                pos = pos+1;
                outline(step).Color = [1,0,0];
            end
        end
        
        %Determine if Done
        button = questdlg('Would you like to label more rois?','Label more rois?','Yes','No','Yes');
        if ~strcmp(button,'Yes')
            break
        end
    end
    ...
    if exist('smallrois','var')
        allLabelled = sort(horzcat(smallrois,selrois));
    elseif ~exist('selrois','var')
        allLabelled = [];
    else
        allLabelled = selrois;
    end
    ...
    [CellList(:).isLabel] = deal(0);
    [CellList(allLabelled).isLabel] = deal(1);
